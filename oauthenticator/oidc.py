"""
Custom Authenticator to use OpenID Connect OAuth with JupyterHub
...
if GITHUB_HOST == 'github.com':
    ...
    GITHUB_API = '%s/api/' % GITHUB_HOST

class GitHubMixin(OAuth2Mixin):
    _OAUTH_AUTHORIZE_URL = "https://%s/oauth/authorize" % GITHUB_HOST
    _OAUTH_ACCESS_TOKEN_URL = "https://%s/oauth/access_token" % GITHUB_HOST
...
class GitHubOAuthenticator(OAuthenticator):
    ...
    def authenticate(self, handler, data=None):
        ...
        url = url_concat("https://%s/oauth/access_token" % GITHUB_HOST,
                         params)
